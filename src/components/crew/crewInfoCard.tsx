...
export default function CrewInfoCard({ crew, isMutating, inlineMessage, onRotateInviteCode }: Props) {
    ...
    <div className="bg-navy-8 rounded-xl px-5 py-5 border border-navy-15 shadow-sm">
      ...
      {crew.inviteCode && (
        <div className="mt-4 pt-4 border-t border-navy-15">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {isLeader && (
              <button
                ...
                disabled={isMutating}
                className="text-f12 text-navy-70 border border-navy-15 rounded-pill px-3 py-1 flex-shrink-0 disabled:opacity-40 active:opacity-70 transition-opacity"
              >
                재발급
              </button>
            )}
          ...
          {inlineMessage && (
            <p className="text-f12 text-navy-70 mt-1">{inlineMessage}</p>
          )}
        </div>
      )}
    </div>
